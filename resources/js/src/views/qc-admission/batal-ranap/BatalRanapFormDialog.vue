  ...
  <VDialog :model-value="modelValue" max-width="600" persistent scrollable @update:model-value="close">
    <VCard rounded="xl" class="dlg-card overflow-hidden">
      ...
      <div class="dlg-banner">
        <div class="db-blob db-blob--1" />
        <div class="db-blob db-blob--2" />
        <div class="db-inner">
          <div class="db-icon">
        ...
        <div class="px-5 py-4 d-flex flex-column gap-4">
          ...
          <div v-if="!editItem" class="auto-trigger-banner">
            ...
            <span class="text-caption">
              Setelah disimpan, gunakan aksi <strong>Verifikasi</strong> pada data untuk konfirmasi dan update status
              bed management IGD.
            </span>
          </div>
          ...
          <div class="form-section form-section--error">
            ...
            <div class="fs-body">
              <!-- Search -->
              <VAutocomplete v-model="form.no_reg" v-model:search="noRegSearch" :items="pasienStore.optionList"
                item-title="title" item-value="value" label="Cari No. Reg / No. MR / Nama Pasien *" variant="outlined"
                density="compact" prepend-inner-icon="ri-search-line" clearable no-filter
                :loading="noRegLoading || pasienStore.loading" no-data-text="Ketik min. 2 karakter..." class="mb-3"
                hide-details="auto">
                <template #item="{ item, props: iProps }">
                  <VListItem v-bind="iProps" class="py-2">
                    ...
                    <VListItemSubtitle class="d-flex flex-wrap gap-1 mt-1">
                      ...
                      <VChip size="x-small" color="secondary" variant="tonal" label>{{ item.raw?.data?.ket_bayar }}
                      </VChip>
              ...
              <Transition name="slide-down">
                <div v-if="hasPasien" class="info-chip-grid">
                  <div class="icg-cell icg-cell--span2">
                    <span class="icg-lbl">
                      <VIcon icon="ri-user-3-line" size="10" class="me-1" />Nama Pasien
                    </span>
                    <span class="icg-val icg-val--accent">{{ form.nama_pasien || '—' }}</span>
                  </div>
                  <div class="icg-cell">
                    <span class="icg-lbl">
                      <VIcon icon="ri-id-card-line" size="10" class="me-1" />No. MR
                    </span>
                    <span class="icg-val icg-val--mono">{{ form.no_mr || '—' }}</span>
                  </div>
                  <div class="icg-cell">
                    <span class="icg-lbl">
                      <VIcon icon="ri-shield-user-line" size="10" class="me-1" />Jaminan
                    </span>
                    <span class="icg-val">{{ form.jaminan || '—' }}</span>
                  </div>
                  <div class="icg-cell">
                    <span class="icg-lbl">
                      <VIcon icon="ri-door-line" size="10" class="me-1" />Ruangan
                    </span>
                    <span class="icg-val">{{ form.ruangan || '—' }}</span>
                  </div>
                  <div class="icg-cell icg-cell--span2">
                    <span class="icg-lbl">
                      <VIcon icon="ri-stethoscope-line" size="10" class="me-1" />Diagnosa
                    </span>
                    <span class="icg-val">{{ form.diagnosa || '—' }}</span>
                  </div>
                  <div class="icg-cell">
                    <span class="icg-lbl">
                      <VIcon icon="ri-calendar-check-line" size="10" class="me-1" />Tgl. Daftar
                    </span>
                    <span class="icg-val">{{ form.tgl_daftar || '—' }}</span>
                  </div>
                  <div class="icg-cell">
                    <span class="icg-lbl">
                      <VIcon icon="ri-time-line" size="10" class="me-1" />Jam Daftar
                    </span>
          ...
          <div class="form-section form-section--warning">
            ...
            <div class="fs-body">
              <VSelect v-model="form.keterangan_batal" :items="masterStore.keteranganBatalList"
                label="Keterangan Batal *" variant="outlined" density="compact"
                prepend-inner-icon="ri-close-circle-line" clearable class="mb-3" hide-details="auto" />
              <VSelect v-model="form.status_ok" :items="masterStore.statusOkList" label="Status OK (Opsional)"
                variant="outlined" density="compact" clearable class="mb-3" hide-details="auto"
                :prepend-inner-icon="form.status_ok === 'Bedah' ? 'ri-surgical-mask-line' : form.status_ok === 'Non Bedah' ? 'ri-hospital-line' : 'ri-question-line'" />
              <VTextField v-model="form.ketersediaan_kamar" label="Ketersediaan Kamar Saat Ini" variant="outlined"
                density="compact" prepend-inner-icon="ri-door-line" class="mb-3" hide-details="auto" />
              <VTextField v-model="form.diagnosa" label="Diagnosa" variant="outlined" density="compact"
                prepend-inner-icon="ri-stethoscope-line" hide-details="auto" />
            </div>
          </div>
          ...
          <div class="form-section form-section--info">
            ...
            <div class="fs-body">
              <VAutocomplete v-model="form.petugas" :items="pegawaiStore.namaList" label="Petugas *" variant="outlined"
                density="compact" prepend-inner-icon="ri-nurse-line" clearable class="mb-3" hide-details="auto"
                :loading="pegawaiStore.loading" no-data-text="Memuat petugas..." />
              <VTextarea v-model="form.note" label="Catatan Tambahan" variant="outlined" density="compact" rows="2"
                auto-grow prepend-inner-icon="ri-sticky-note-line" hide-details="auto" />
            </div>
          </div>
      ...
      <div class="dlg-footer">
        ...
        <VBtn color="error" rounded="lg" class="flex-grow-1" prepend-icon="ri-save-line"
...
.form-section--error:focus-within {
  border-color: rgba(var(--v-theme-error), 0.35);
  box-shadow: 0 0 0 3px rgba(var(--v-theme-error), 0.08);
}

.form-section--primary:focus-within {
  border-color: rgba(var(--v-theme-primary), 0.35);
  box-shadow: 0 0 0 3px rgba(var(--v-theme-primary), 0.08);
}

.form-section--warning:focus-within {
  border-color: rgba(var(--v-theme-warning), 0.35);
  box-shadow: 0 0 0 3px rgba(var(--v-theme-warning), 0.08);
}

.form-section--info:focus-within {
  border-color: rgba(var(--v-theme-info), 0.35);
  box-shadow: 0 0 0 3px rgba(var(--v-theme-info), 0.08);
}
